...
with open('flickr_ground.txt') as file:
	all_cnt = 0
	for line in file:
		parts = line.split('\t')
		if len(parts) == 1:
		...
		cc = center.split('/')
		new_cent = cc[1] + '/' + cc[0]
		test_terms[term] = (parts[2], parts[3], new_cent)
...
terms = {}
with open('flickr_ringq.txt') as f:
# 	acc = 0
# 	cnt = 0
# 	for line in f:
# 		parts = line.split('\t\t')
...
# 	print(acc)
	...
	for line in f:
		...
		if term in test_terms:
			...
			# print('{}\t{}\t{}'.format(term, ct, sum_diff_c * 100))
			# print('{}\t{}\t{}'.format(term, 'correct', ct ))
			# print('{}\t{}'.format(at,(math.fabs(at - alpha)/at)))
